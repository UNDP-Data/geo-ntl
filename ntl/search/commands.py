...
import click
from ntl.search.orbital import VIIRSNavigator, search_granules, async_search_granules
...
@click.pass_obj
def search(state, satellites, target_date, bbox, cmask):
    ...
    table = Table(title=f"VIIRS satellites granules for the night of  {target_date.date()} covering {bbox}", title_style="bold yellow")
...
@click.pass_obj
async def async_search(state, satellites, target_date, bbox, cmask):
    #"""List the specific 85.4s data segments (granules) for a pass."""
    """Search VIIRS satellites granules for a given night and a geographic area."""
